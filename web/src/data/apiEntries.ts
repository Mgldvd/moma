...
export const API_ENTRIES: ApiEntryData[] = [
  ...
  },
  {
    ...
    signature: 'moma title-sub "Deployment" "Production environment" [--success|--error|--warning|--info] [--color color] [--icon char] [--no-icon] [--border mirror|line|open] [--width number] [--max-width number]',
    searchText: 'moma-title-sub secondary workflow header subtitle icon marker border mirror line open success error warning info',
    group: 'visual',
    examples: [
    ...
    options: [
      { flag: '--success | --error | --warning | --info', description: 'Sets the rule/text color and the left marker to the matching semantic icon (✔, ✖, !, →).' },
      { flag: '--color <color>', description: 'Sets the rule and text color. Defaults to the primary color (cyan) when omitted.' },
      { flag: '--icon <char>', description: 'Sets the left marker. Defaults to ▪.' },
      { flag: '--no-icon', description: 'Drops the left marker in favor of blank filler (there is no left box edge to fall back to here).' },
      { flag: '--border mirror | line | open', description: 'Controls the closing line: open (default) is a bare underline, line closes it with ┘, and mirror also closes it with ┘ and repeats the left marker at the end of the text line.' },
  ...
  },
  {
    ...
    kind: 'Heading component',
    name: 'sub-title',
    descriptionHtml: 'Rule-first secondary heading: the same marker/padding line as <code>moma-title-sub</code>, but with the rule printed above the text instead of below it, and no closing row.',
    signature: 'moma sub-title "Moma" "Terminal UI library" [--success|--error|--warning|--info] [--color color] [--icon char] [--no-icon] [--border mirror|line|open] [--min-width number] [--width number] [--max-width number]',
    searchText: 'moma-sub-title rule-first secondary workflow header subtitle icon marker border mirror line open success error warning info',
    group: 'visual',
    examples: [
    ...
    options: [
      ...
      { flag: '--icon <char>', description: 'Sets the left marker. Defaults to ▪.' },
      { flag: '--no-icon', description: 'Drops the left marker in favor of blank filler (there is no left box edge to fall back to here).' },
      { flag: '--border mirror | line | open', description: 'Controls the rule above the text: open leaves it without a ┐, line closes it with ┐, and mirror (default) also closes it with ┐ and repeats the left marker at the end of the text line.' },
      { flag: '--min-width <number>', description: 'Minimum width in characters. Defaults to 35.' },
      { flag: '--width <number>', description: 'Forces an exact width, overriding automatic sizing. Unset by default.' },
      { flag: '--max-width <number>', description: 'Caps the width when the content would be wider. Unset by default.' },
    ],
  },
  ...
  },
  {
    ...
    options: [
      { flag: '--success | --error | --warning | --info', description: 'Sets the rule color and the marker to the matching semantic icon (✔, ✖, !, →).' },
      { flag: '--color <color>', description: 'Sets the rule color. Defaults to the primary color (cyan) when omitted.' },
      { flag: '--icon <char>', description: 'Sets the marker. Defaults to ▪.' },
      { flag: '--no-icon', description: 'Drops the marker in favor of blank filler (there is no left box edge to fall back to here).' },
      { flag: '--border mirror | line | open', description: 'open (default) prints just the rule; line and mirror both frame it with a bare ┌ above and └ below.' },
      { flag: '--min-width <number>', description: 'Minimum rule length in characters. Defaults to 35.' },
      { flag: '--width <number>', description: 'Forces an exact rule length, overriding automatic sizing. Unset by default.' },
      { flag: '--max-width <number>', description: 'Caps the rule length when it would be wider. Unset by default.' },
      { flag: '--no-color', description: 'Strips ANSI color codes entirely.' },
    ],
  },
  {
    ...
    signature: 'moma label "TEXT HERE" [--width number] [--min-width number] [--max-width number] [--color color] [--icon symbol] [--edge top|bottom] [--border line|open]',
    searchText: 'moma-label decorated input header separator text width color semantic edge top bottom border line open corner',
    group: 'visual',
    examples: [
    ...
    options: [
      ...
      { flag: '--min-width <number>', description: 'Minimum label width in characters. Defaults to 35.' },
      ...
      { flag: '--edge top | bottom', description: 'Which corner the rule uses: top (default) draws ┌/┐, bottom draws └/┘.' },
      { flag: '--border line | open', description: 'line (default) closes the far end with the matching corner; open leaves it as a bare rule.' },
  ...
  },
  {
    ...
    name: 'prompt',
    descriptionHtml: 'Free-text prompt with <code>moma-title-sub</code>\'s own marker + underline look instead of <code>moma-input</code>\'s boxed field - the two share the same read, validation, and secret-masking machinery, and differ only in how the question is drawn.',
    signature: 'moma prompt "<question>" [--success|--error|--warning|--info] [--color color] [--icon char] [--no-icon] [--border mirror|line|open] [--cursor char] [--default value] [--secret] [--mask char] [--required] [--trim] [--min-width number] [--width number] [--max-width number]',
    searchText: 'moma-prompt interactive question free text read value default required trim secret mask icon marker border mirror line open cursor',
    group: 'interactive',
    examples: [
    ...
    options: [
      { flag: '--success | --error | --warning | --info', description: 'Sets the color and the marker to the matching semantic icon (✔, ✖, !, →).' },
      { flag: '--color <color>', description: 'Sets the prompt color. Defaults to the warning color (yellow) when omitted.' },
      { flag: '--icon <char>', description: 'Sets the left marker. Defaults to ▪.' },
      { flag: '--no-icon', description: 'Drops the left marker in favor of blank filler.' },
      { flag: '--border mirror | line | open', description: 'Controls the underline: open (default) is a bare underline, line closes it with ┘, and mirror also closes it with ┘ and repeats the left marker at the end of the question line.' },
      { flag: '--cursor <char>', description: 'The symbol the answer is typed after. Defaults to "❯".' },
      { flag: '--default <value>', description: 'Shown as "[value]" after the question, and substituted if the typed response is empty. Empty by default.' },
      { flag: '--secret', description: 'Masks each typed character instead of echoing it.' },
      { flag: '--mask <char>', description: 'The character echoed for each keystroke when --secret is used. Defaults to "*".' },
      { flag: '--required', description: 'Re-prompts until a non-empty value is produced.' },
      { flag: '--trim', description: 'Strips leading/trailing whitespace from the entered value.' },
      { flag: '--min-width <number>', description: 'Minimum underline width in characters. Defaults to 30.' },
      { flag: '--width <number>', description: 'Forces an exact width, overriding automatic sizing. Unset by default.' },
      { flag: '--max-width <number>', description: 'Caps the width when the content would be wider. Unset by default.' },
    ],
  },
