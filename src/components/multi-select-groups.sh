...
# arguments. Group headings cost extra lines, so when windowed and the full
# layout would not fit the terminal, this switches to a compact layout
# instead: headings are dropped, each group's All row is labeled with its
# group name inline, and blank separators are kept. How many rows are
# visible (and how many of those are blank separators) varies with scroll
# position, so the visible rows are padded with trailing blank filler up to
# a session-constant budget; this keeps the total printed line count fixed
# for the life of one component invocation, which is what makes
# _moma_term_move_up's plain line-counting correct on every redraw.
_moma_render_multi_select_groups() {
  ...
  # Every row list has exactly one Select All row and one All row per
  # group, so the unwindowed blank-separator count is always this, with no
  # need to scan for it.
  local full_blank_count=$((1 + num_groups))

  local compact=false window_start=0 window_count="$row_count" \
    indicator_lines=0 blank_count="$full_blank_count" pad_lines=0
  if $windowed; then
    local term_height content_budget
    term_height="$(_moma_term_height)"
    # Reserve one row of headroom below the shell prompt: filling every
    # last row leaves nothing to absorb the prompt/command-echo line above
    # the render, so the terminal's own scroll can push the title (this
    # component's first printed line) out of view before the first redraw
    # even runs.
    term_height=$((term_height - 1))
    ((term_height < 1)) && term_height=1
    if ((full_lines > term_height)); then
      compact=true
      # Space left for the indicator (if needed) plus rows and their blank
      # separators, once the header and footer are accounted for.
      content_budget=$((term_height - 3))
      ((content_budget < 1)) && content_budget=1
      if ((row_count + full_blank_count > content_budget)); then
        indicator_lines=1
        local rows_budget=$((content_budget - 1))
        ((rows_budget < 1)) && rows_budget=1
        local max_visible window
        max_visible="$rows_budget"
        ...
        if ((window_count + blank_count > rows_budget)); then
          max_visible=$((rows_budget - blank_count))
        ...
        # Pad up to the fixed budget so the total row-area line count never
        # depends on how many boundaries this particular window contains.
        pad_lines=$((rows_budget - window_count - blank_count))
        ((pad_lines < 0)) && pad_lines=0
      fi
      # Otherwise the compact layout already fits without windowing at
      # all: row_count and full_blank_count are both independent of
      # active_row, so that stays true, unwindowed, for the whole session.
    fi
  fi
  ...
  if $compact; then
    if $redraw; then
      _moma_term_move_up \
        "$((2 + indicator_lines + window_count + blank_count + pad_lines + 1))"
    fi
    ...
    local pad
    for ((pad = 0; pad < pad_lines; pad++)); do
      _moma_select_render_blank "$redraw"
    done
