...
- Record every notable change under the `Unreleased` heading in `CHANGELOG.md` as it lands. When cutting a release, bump `MOMA_VERSION` in `src/core/version.sh`, retitle `Unreleased` to the matching `vMAJOR.MINOR.PATCH`, and update the version badge (`data-moma-version` and its `vX.Y.Z` text) in `web/index.html`; `tests/smoke.sh` requires the badge, `MOMA_VERSION`, and the release tag to match. See [Versioning](README.md#versioning) for the policy.
