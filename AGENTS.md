...
- Treat `src/lib/*.sh`, `docs/moma-help.md`, `docs/moma-preview.md`, and `preview/{index.html,styles.css,app.js}` as sources. `dist/moma` is generated; never edit it directly.
...
- The version is generated from `MOMA_VERSION` in `build.sh`.
...
- When adding or removing a public `moma-*` function, update `tests/smoke.sh`'s `expected_functions`, `preview/index.html` (`data-api`), `src/lib/README.md`, and `example.sh`; the smoke suite requires all four to match.
